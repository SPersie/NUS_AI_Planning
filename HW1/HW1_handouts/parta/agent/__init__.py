...
class GeneratePDDL_Stationary :
    ...
    def generateInitString(self) :
        ...
        ### add cars block trails
        for car in self.state.cars:
            car_x = car.position.x
            car_y = car.position.y
            opt += '(blocked {})'.format(self.grid_cell_list[car_x * self.num_lanes + car_y])

        ### add others constrats whether up down next
        for i in range(1, self.width):
            for j in range(self.num_lanes):
                pos1 = i * self.num_lanes + j
                pos2 = (i - 1) * self.num_lanes + j
                ## add forward next
                opt += '(forward_next {} {})'.format(self.grid_cell_list[pos1], self.grid_cell_list[pos2])

                ## add up next
                if j > 0:
                    opt += '(up_next {} {})'.format(self.grid_cell_list[pos1], self.grid_cell_list[pos2 - 1])
                else:
                    opt += '(up_next {} {})'.format(self.grid_cell_list[pos1], self.grid_cell_list[pos2])

                ## add down next
                if j < self.num_lanes - 1:
                    opt += '(down_next {} {})'.format(self.grid_cell_list[pos1], self.grid_cell_list[pos2 + 1])
                else:
                    opt += '(down_next {} {})'.format(self.grid_cell_list[pos1], self.grid_cell_list[pos2])
    ...
    def generateGoalString(self) :
        ...
        opt = '(at pt{}pt{} agent1)'.format(self.state.finish_position.x, self.state.finish_position.y)   
        return opt
