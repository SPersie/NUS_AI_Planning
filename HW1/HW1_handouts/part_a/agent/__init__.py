...
class GeneratePDDL_Stationary :
    ...
    def generateInitString(self) :
        ...
        for gx in range(1,self.width):
            for gy in range(self.num_lanes):
                ans += '(forward_next {} {})'.format(self.grid_cell_list[gx*self.num_lanes+gy], self.grid_cell_list[(gx-1)*self.num_lanes + gy])
                if gy > 0:
                    ans += '(up_next {} {})'.format(self.grid_cell_list[gx*self.num_lanes+gy], self.grid_cell_list[(gx-1)*self.num_lanes + gy-1])
                else:
                    ans += '(up_next {} {})'.format(self.grid_cell_list[gx*self.num_lanes+gy], self.grid_cell_list[(gx-1)*self.num_lanes + gy])
                if gy < self.num_lanes-1:
                    ans += '(down_next {} {})'.format(self.grid_cell_list[gx*self.num_lanes+gy], self.grid_cell_list[(gx-1)*self.num_lanes + gy+1])
                else:
                    ans += '(down_next {} {})'.format(self.grid_cell_list[gx*self.num_lanes+gy], self.grid_cell_list[(gx-1)*self.num_lanes + gy])
        return ans
