...
test_config = [{'lanes' : [LaneSpec(6, [-2, -2])] *2 + [LaneSpec(6, [-5, -5])] *2 +
               ...
               {'lanes' : [LaneSpec(6, [-2, -2])] *2 + [LaneSpec(4, [-3, -3])] *2 + 
                          ...
                          [LaneSpec(6, [-2, -2])] *2 + [LaneSpec(4, [-3, -3])] *2, 'width' :35, 'seed' : 101},
               ...
               {'lanes' : [LaneSpec(7, [-1, -1])] *10,'width' :20, 'seed': 21}]
...
class GeneratePDDL_Stationary :
        ...
        for t in range(self.width) :
            for w in range(self.width) :
                for lane in range(self.num_lanes) :
                    self.grid_cell_list.append("pt{}pt{}_{}".format(w, lane, t))
    ...
    def generateInitString(self) :
        ...
        '''  
        ax, ay = self.state.agent.position.x, self.state.agent.position.y
        idx = ax * self.num_lanes + ay
        ans = '(at {} agent1)'.format(self.grid_cell_list[idx])
        # ans += '(blocked {})'.format(self.grid_cell_list[idx])
        num_grid = self.width * self.num_lanes
        explored = [False] * self.num_lanes * self.width * self.width
    
        for car in self.state.cars:
            car_p = car.position.x
            
            ans += ' (blocked {})'.format(self.grid_cell_list[car.position.x*self.num_lanes + car.position.y])
            for t in range(1, self.width):
                nxt_p = car_p + car.speed_range[0]
                if nxt_p < 0:
                    nxt_p = self.width + nxt_p

                # if car_p > 0 and car_p < self.width-1:
                #     ans += '(blocked_between {} {})'.format(self.grid_cell_list[num_grid*t+i*self.num_lanes + car.position.y])

                if nxt_p < car_p:
                    for i in range(nxt_p,car_p):
                        if not explored[num_grid*t+i*self.num_lanes + car.position.y]:
                            ans += ' (blocked {})'.format(self.grid_cell_list[num_grid*t+i*self.num_lanes + car.position.y])
                            explored[num_grid*t+i*self.num_lanes + car.position.y] = True
                elif nxt_p > car_p:
                    for i in range(car_p):
                        if not explored[num_grid*t+i*self.num_lanes + car.position.y]:
                            ans += ' (blocked {})'.format(self.grid_cell_list[num_grid*t+i*self.num_lanes + car.position.y])
                            explored[num_grid*t+i*self.num_lanes + car.position.y] = True
                    for i in range(nxt_p, self.width):
                        if not explored[num_grid*t+i*self.num_lanes + car.position.y]:
                            ans += ' (blocked {})'.format(self.grid_cell_list[num_grid*t+i*self.num_lanes + car.position.y])
                            explored[num_grid*t+i*self.num_lanes + car.position.y] = True
                elif nxt_p == car_p:
                    if not explored[num_grid*t+nxt_p*self.num_lanes + car.position.y]:
                        ans += ' (blocked {})'.format(self.grid_cell_list[num_grid*t+nxt_p*self.num_lanes + car.position.y])
                        explored[num_grid*t+nxt_p*self.num_lanes + car.position.y] = True
                car_p = nxt_p

        for t in range(self.width-1):
            for gx in range(self.width):
                for gy in range(self.num_lanes):
                    if gx > 0:
                        ans += '(forward1_next {} {})'.format(self.grid_cell_list[num_grid*t+gx*self.num_lanes+gy], 
                                                              self.grid_cell_list[num_grid*(t+1)+(gx-1)*self.num_lanes + gy])
                    if gx > 1:
                        ans += '(forward2_next {} {})'.format(self.grid_cell_list[num_grid*t+gx*self.num_lanes+gy], 
                                                              self.grid_cell_list[num_grid*(t+1)+(gx-2)*self.num_lanes + gy])
                    if gx > 2:
                        ans += '(forward3_next {} {})'.format(self.grid_cell_list[num_grid*t+gx*self.num_lanes+gy], 
                                                              self.grid_cell_list[num_grid*(t+1)+(gx-3)*self.num_lanes + gy])
                    if gx > 0:
                        if gy > 0:
                            ans += '(up_next {} {})'.format(self.grid_cell_list[num_grid*t+gx*self.num_lanes+gy], 
                                                            self.grid_cell_list[num_grid*(t+1)+(gx-1)*self.num_lanes + gy-1])
                        else:
                            ans += '(up_next {} {})'.format(self.grid_cell_list[num_grid*t+gx*self.num_lanes+gy], 
                                                            self.grid_cell_list[num_grid*(t+1)+(gx-1)*self.num_lanes + gy])
                        if gy < self.num_lanes-1:
                            ans += '(down_next {} {})'.format(self.grid_cell_list[num_grid*t+gx*self.num_lanes+gy], 
                                                            self.grid_cell_list[num_grid*(t+1)+(gx-1)*self.num_lanes + gy+1])
                        else:
                            ans += '(down_next {} {})'.format(self.grid_cell_list[num_grid*t+gx*self.num_lanes+gy], 
                                                            self.grid_cell_list[num_grid*(t+1)+(gx-1)*self.num_lanes + gy])
    ...
    def generateGoalString(self) :
        ...
        Hint : Use the defined grid cell objects from generateGridCells and predicates to construct the goal string.
        ...
        for i in range(self.width):
            goal += '(at pt{}pt{}_{} agent1)'.format(self.state.finish_position.x, self.state.finish_position.y, i)
        return '(or ' + goal + ')'
...
def generateDomainPDDLFile(gen):
    ...
    # gen.addPredicate(name="blocked_between", parameters=[("pt1" , "gridcell"),('pt2', 'gridcell')])
    ...
    gen.addAction(name="UP", 
                  ...
                  effect_string="(and (at ?pt2 ?agent)(blocked ?pt2)(not (blocked ?pt1)))")
    ...
    gen.addAction(name="FORWARD1", 
                  parameters = (('agent','agent'),('pt1', 'gridcell'),('pt2', 'gridcell')), 
                  precondition_string = "(and (at ?pt1 ?agent)(not (blocked ?pt2))(forward1_next ?pt1 ?pt2))", 
                  effect_string="(and (at ?pt2 ?agent)(blocked ?pt2)(not (blocked ?pt1)))")
    gen.addAction(name="FORWARD2", 
                  parameters = (('agent','agent'),('pt1', 'gridcell'),('pt2', 'gridcell')), 
                  precondition_string = "(and (at ?pt1 ?agent)(not (blocked ?pt2))(forward2_next ?pt1 ?pt2))", 
                  effect_string="(and (at ?pt2 ?agent)(blocked ?pt2)(not (blocked ?pt1)))")
    gen.addAction(name="FORWARD3", 
                  parameters = (('agent','agent'),('pt1', 'gridcell'),('pt2', 'gridcell')), 
                  precondition_string = "(and (at ?pt1 ?agent)(not (blocked ?pt2))(forward3_next ?pt1 ?pt2))", 
                  effect_string="(and (at ?pt2 ?agent)(blocked ?pt2)(not (blocked ?pt1)))")
    '''
