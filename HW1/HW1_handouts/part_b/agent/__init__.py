...
class GeneratePDDL_Stationary :
    ...
    def generateInitString(self) :
        ...
        for car in self.state.cars:
            ...
            for t in range(1, self.width):
                ...
                if nxt_car_x < car_x:
                    ...
                    tmp = num_cells*t+nxt_car_x*self.num_lanes + car_y
                    if not blocked_cells[tmp]:
                        opt += ' (blocked {})'.format(self.grid_cell_list[tmp])
                        blocked_cells[tmp] = True
                car_x = nxt_car_x
        ...
        for t in range(self.width-1):
            for gx in range(self.width):
                for gy in range(self.num_lanes):
                    if gx > 0:
                        opt += '(forward-1_next {} {})'.format(self.grid_cell_list[num_cells*t+gx*self.num_lanes+gy], 
                                                               self.grid_cell_list[num_cells*(t+1)+(gx-1)*self.num_lanes + gy])
                    if gx > 1:
                        opt += '(forward-2_next {} {})'.format(self.grid_cell_list[num_cells*t+gx*self.num_lanes+gy], 
                                                               self.grid_cell_list[num_cells*(t+1)+(gx-2)*self.num_lanes + gy])
                    if gx > 2:
                        opt += '(forward-3_next {} {})'.format(self.grid_cell_list[num_cells*t+gx*self.num_lanes+gy], 
                                                               self.grid_cell_list[num_cells*(t+1)+(gx-3)*self.num_lanes + gy])
                    if gx > 0:
                        if gy > 0:
                            opt += '(up_next {} {})'.format(self.grid_cell_list[num_cells*t+gx*self.num_lanes+gy], 
                                                            self.grid_cell_list[num_cells*(t+1)+(gx-1)*self.num_lanes + gy-1])
                        else:
                            opt += '(up_next {} {})'.format(self.grid_cell_list[num_cells*t+gx*self.num_lanes+gy], 
                                                            self.grid_cell_list[num_cells*(t+1)+(gx-1)*self.num_lanes + gy])
                        if gy < self.num_lanes-1:
                            opt += '(down_next {} {})'.format(self.grid_cell_list[num_cells*t+gx*self.num_lanes+gy], 
                                                              self.grid_cell_list[num_cells*(t+1)+(gx-1)*self.num_lanes + gy+1])
                        else:
                            opt += '(down_next {} {})'.format(self.grid_cell_list[num_cells*t+gx*self.num_lanes+gy], 
                                                              self.grid_cell_list[num_cells*(t+1)+(gx-1)*self.num_lanes + gy])
        ...
        #             ### add forward action
        #             if i > 0:
        #                 opt += '(forward1_next {} {})'.format(self.grid_cell_list[pos1],
        #                     self.grid_cell_list[num_cells * (t + 1) + (i - 1) * self.num_lanes + j])

        #             if i > 1:
        #                 opt += '(forward2_next {} {})'.format(self.grid_cell_list[pos1],
        #                     self.grid_cell_list[num_cells * (t + 1) + (i - 2) * self.num_lanes + j])

        #             if i > 2:
        #                 opt += '(forward3_next {} {})'.format(self.grid_cell_list[pos1],
    ...
    def generateGoalString(self) :
        ...
        opt = ''
        for i in range(self.width):
            opt += '(at pt{}pt{}t{} agent1)'.format(self.state.finish_position.x, self.state.finish_position.y, i)
