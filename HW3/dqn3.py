...
class ReplayBuffer():
    ...
    def sample(self, batch_size):
        ...
        # c, h, w = sampling[0][0].shape
        
        # states = np.concatenate([i[0] for i in sampling]).reshape(batch_size, c, h, w)
        # actions = np.concatenate([i[1] for i in sampling]).reshape(batch_size, 1)
        # rewards = np.concatenate([i[2] for i in sampling]).reshape(batch_size, 1)
        # next_states = np.concatenate([i[3] for i in sampling]).reshape(batch_size, c, h, w)
        # dones = np.concatenate([i[4] for i in sampling]).reshape(batch_size, 1)

        # states = torch.from_numpy(states).double().to(device)
        # actions = torch.from_numpy(actions).to(device)
        # rewards = torch.from_numpy(rewards).double().to(device)
        # next_states = torch.from_numpy(next_states).double().to(device)
        # dones = torch.from_numpy(dones).double().to(device)
...
class BaseAgent(Base):
    def act(self, state, epsilon=0.0):
        ...
        #     return np.random.randint(low=0, high=self.feature_size)
...
def compute_loss(model, target, states, actions, rewards, next_states, dones):
    ...
    q_target_value = ((q_next_state_values * gamma) * (1-dones.int()).float()[indices, [0]]) + rewards[indices, [0]]
