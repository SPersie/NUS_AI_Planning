...
class ReplayBuffer():
    ...
    def sample(self, batch_size):
        ...
        batches = Transition(*zip(*sampling))

        states = torch.tensor(batches.state).float().to(device)
        actions = torch.tensor(batches.action).to(device)
        rewards = torch.tensor(batches.reward).float().to(device)
        next_states = torch.tensor(batches.next_state).float().to(device)
        dones = torch.tensor(batches.done).float().to(device)
    ...
    def act(self, state, epsilon=0.0):
        if not isinstance(state, torch.FloatTensor):
            ...
            if np.random.random() < epsilon:
                ...
                actual = self.forward(state)
                return torch.argmax(actual).item()
        '''
...
def compute_loss(model, target, states, actions, rewards, next_states, dones):
    ...
    # print(dones)
    # print(rewards.shape)
    q_current = model.forward(states).gather(1,actions)
    q_next_max = target.forward(next_states).max(1)[0].detach()

    index = np.arange(len(dones))

    a = (1-dones.int()).float()
    q_value = rewards[index, [0]] + a[index, [0]] * gamma * q_next_max
    return F.smooth_l1_loss(q_current, q_value.unsqueeze(1)) 
    pass
...
def train(model_class, env):
    ...
    # use pretrained
    pre_path = os.path.join(script_path, 'model_1.pt')
    model_p = get_model(pre_path)
    ...
    # for episode in range(10):
        ...
        for t in range(t_max):
            ...
            if action == 0:
                action_r = 5
            # if not(done) and (action == 0 or action == 2 or action == 3 or action == 4):
            #     if action == 0:
            #         action_r = 0.1
            #     elif action == 2:
            #         action_r = 0.3
            ...
            reward_p = reward + (1 / distace) + time + action_r
        ...
        if episode % print_interval == 0 and episode > 0:
            print("[Episode {}]\tavg rewards : {:.3f},\tavg loss: : {:.6f},\tbuffer size : {},\tepsilon : {:.1f}%".format(
                            episode, np.mean(rewards[-20:]), np.mean(losses[print_interval*10:]), len(memory), epsilon*100))
            # if np.sum(state[1]) == 0:
            print(x_, y_)
            env.render()
    return model
