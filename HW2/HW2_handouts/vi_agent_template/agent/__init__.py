...
def value_iteration(trn_fn, rwd_fn, gamma):
    ...
    # predefine variables
    k = 0
    num_sates = rwd_fn.shape[0]
    values = [np.zeros((num_sates,))]
    ...
    while True:
        ...
        next_k = np.zeros((num_sates,))

        for i in range(num_sates):
            ipt_a = trn_fn[:, i, :]
            # print(ipt_a.shape)

            rwd = rwd_fn[i, :].T.reshape((num_sates, 1))
            last_it = gamma * values[k - 1].reshape((num_sates, 1))
            ipt_b = rwd + last_it

            value = np.array(matmul(ipt_a, ipt_b))
            # print(value.shape)
            next_k[i] = np.max(value)
        values.append(next_k)

        # abort when converge
        if (values[k] - values[k-1]).all() < delta:
            value_fn = values[k]
            break

    # back prop to get policy
    for i in range(num_sates):
        ipt_a = trn_fn[:, i, :]

        rwd = rwd_fn[i, :].T.reshape((num_sates, 1))
        last_it = gamma * value_fn.reshape((num_sates, 1))
        ipt_b = rwd + last_it

        value = np.array(matmul(ipt_a, ipt_b))

        policy[i] = np.argmax(value)
