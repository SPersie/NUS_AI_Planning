...
class MonteCarloTreeSearch:
    ...
    def chooseBestActionNode(self, node, explorationValue):
        ...
        for child in node.children.values():
            ...
            '''
            # try:
            q = child.totalReward / child.numVisits
            bound = explorationValue * math.sqrt(math.log(node.numVisits) / child.numVisits)
            pai = q + bound
            # except:
            #     pai = 0
...
if not SUBMISSION:
    ...
    test_case_number = 2  #Change the index for a different test case
